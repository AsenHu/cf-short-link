...
export const onRequestGet = async (context: { request: Request, env: Env }) => {
    ...
    let suffix = query.toString();
    if (suffix) suffix = '/?' + suffix;
    // 检查前缀是否是 p
    let prefix = url.slice(0, 1).toLowerCase();
    if (prefix === 'p') {
        // 如果是 p，重定向到 https://asen.page/ + p 后面的部分
        return Response.redirect('https://asen.page/' + url.slice(1) + suffix, 301);
    }
    ...
    if (prefix === 'av') {
        ...
        return Response.redirect('https://www.bilibili.com/video/av' + url.slice(2) + suffix, 301);
    }

    // 检查前缀是否是 bv
    if (prefix === 'bv') {
        // 如果是 bv，重定向到 https://www.bilibili.com/video/BV + url 后面的部分
        return Response.redirect('https://www.bilibili.com/video/BV' + url.slice(2) + suffix, 301);
    }

    // 检查前缀是否是 cv
    if (prefix === 'cv') {
        // 如果是 cv，重定向到 https://www.bilibili.com/read/cv + url 后面的部分
        return Response.redirect('https://www.bilibili.com/read/cv' + url.slice(2) + suffix, 301);
    }

    if (suffix) suffix = '&' + suffix.slice(2);
    // 检查前缀是否是 yt
    if (prefix === 'yt') {
        // 如果是 yt，重定向到 https://www.youtube.com/watch?v= + url 后面的部分
        return Response.redirect('https://www.youtube.com/watch?v=' + url.slice(2) + suffix, 301);
    }
